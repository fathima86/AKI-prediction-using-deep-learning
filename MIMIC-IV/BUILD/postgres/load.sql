...
--  psql "dbname=<DBNAME> user=<USER>" -v mimiciv_data_dir=<PATH TO DATA DIR> -f load.sql
-- The script assumes the files are in the core, hosp, and icu subfolders of mimiciv_data_dir

-- core schema
\cd :mimiciv_data_dir/core

\COPY mimiciv_core.admissions FROM admissions.csv DELIMITER ',' CSV HEADER NULL '';
\COPY mimiciv_core.patients FROM patients.csv DELIMITER ',' CSV HEADER NULL '';
\COPY mimiciv_core.transfers FROM transfers.csv DELIMITER ',' CSV HEADER NULL '';
