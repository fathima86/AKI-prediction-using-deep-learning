...
--  Elective surgery (sourced FROM `physionet-data.mimiciv_core.admissions` and SERVICES)
--  Pre-ICU in-hospital length of stay (sourced FROM `physionet-data.mimiciv_core.admissions` and ICUSTAYS)
--  Age (sourced FROM `physionet-data.mimiciv_core.patients`)
...
FROM `physionet-data.mimiciv_icu.icustays` ie
inner join `physionet-data.mimiciv_core.admissions` adm
  on ie.hadm_id = adm.hadm_id
inner join `physionet-data.mimiciv_core.patients` pat
